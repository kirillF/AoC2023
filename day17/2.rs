...
fn min_cost(matrix: &[Vec<i32>]) -> i32 {
    let m = matrix.len();
    let n = matrix[0].len();
    let mut dp = vec![vec![vec![vec![i32::MAX; 11]; 4]; n]; m];
    ...
    while let Some(Reverse((cost, x, y, dir, streak))) = pq.pop() {
        ...
        if cost > min_cost && streak >= 4 {
            continue;
        }
        if x == m - 1 && y == n - 1 && streak >= 4 {
        ...
        if streak < 4 {
            ...
            if (0..m as i32).contains(&nx) && (0..n as i32).contains(&ny) {
                ...
                if ncost < dp[nx][ny][dir][streak + 1] {
                    dp[nx][ny][dir][streak + 1] = ncost;
                    pq.push(Reverse((ncost, nx, ny, dir, streak + 1)));
                }
            }
        } else {
            for &i in [dir, (dir + 1) % 4, (dir + 3) % 4].iter() {
                ...
                if (0..m as i32).contains(&nx) && (0..n as i32).contains(&ny) {
                    ...
                    if nstreak <= 10 && ncost < dp[nx][ny][i][nstreak] {
